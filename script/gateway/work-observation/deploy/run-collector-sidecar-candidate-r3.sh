#!/bin/sh
# Candidate-r3 sidecar launcher used by the reproducible GPU unit.  The
# collector shares the current Nginx network namespace; the container itself
# remains loopback-only and no production Guard/Auto endpoint is contacted.
set -eu

instance=${1:-}
case "$instance" in
  tokyo) nginx_container=tokyo-sub2api-proxy; config=/opt/work-observation/candidate-r3/config/tokyo.json; store=/data/work-observation/tokyo ;;
  us) nginx_container=us-sub2api-proxy; config=/opt/work-observation/candidate-r3/config/us.json; store=/data/work-observation/us ;;
  *) echo 'usage: run-collector-sidecar-candidate-r3.sh tokyo|us' >&2; exit 2 ;;
esac

binary=${WO_BINARY:-/data/work-observation-lab/candidate-r2/work-observation/observe}
...
name="work-observation-collector-${instance}-candidate-r3"
