# 方法 40｜CFPB 投诉主题与异常看板

> **一页结论：**面向社区银行、信用合作社、fintech 客服/产品/合规团队，用「一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要」先收费验证。启动现金成本 ¥100–500，目标是 主题覆盖、异常提前量、人工确认率、客户采取行动数。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 公共数据情报 |
| 买方 | 社区银行、信用合作社、fintech 客服/产品/合规团队 |
| 当前痛点 | 公开投诉量大，团队难发现产品、问题与公司的主题变化 |
| 可交付结果 | 按产品、问题、时间和公司聚合，显示样本量、原始叙述片段和异常而不作裁定 |
| 最小试点 | 一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要 |
| 工具栈 | CFPB CSV/API + Python/Sheets + Looker Studio |
| 启动成本 | ¥100–500（不含自己的人工） |
| 时间 | 10–21 天 |
| 技能 | 数据分析、主题编码、金融投诉边界、可视化 |
| 参考测试报价 | ¥2,000–8,000/报告；¥999–3,999/月 |
| 最小验证 | 先交一个产品的公开样板；3 个团队访谈中 1 个预付 ¥2,000 报告费 |
| 综合分 | 3.75/5；需求证据 4 / 验证速度 3 / 低成本 4 / 复购性 4 / 自动化杠杆 5 / 获客可达 3 / 风险可控 3 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **公开投诉量大，团队难发现产品、问题与公司的主题变化** 变成一个买方能验收的固定范围结果：**按产品、问题、时间和公司聚合，显示样本量、原始叙述片段和异常而不作裁定**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。

### 当前市场证据

- **D13｜[CFPB Consumer Complaint Database](https://www.consumerfinance.gov/data-research/consumer-complaints/)：**CFPB 允许自由使用 CSV/API 做分析；投诉叙述已脱敏，98% 被转交企业的投诉获得及时答复。 **使用边界：**投诉不是监管裁定，近期数据可能仍在审核。
...
企业 VoC 平台能接内部数据；你的差异是公开 CFPB 基准和固定产品线。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 D13、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**CFPB 投诉主题与异常看板 30 天验证包
- **交付：**一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**产品、问题词典、公司、时间窗口、异常阈值、禁用个体字段
- **验收指标：**主题覆盖、异常提前量、人工确认率、客户采取行动数
- **参考报价：**¥2,000–8,000/报告；¥999–3,999/月

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 份报告；归一化校验：1 个该情景订单组合×¥2,000=¥2,000；模型合计=¥2,000 | ¥2,000 |
| 中性 | 中性 3 份报告加 4 个订阅；归一化校验：1 个该情景订单组合×¥12,000=¥12,000；模型合计=¥12,000 | ¥12,000 |
| 乐观 | 乐观 6 份报告加 12 个订阅；归一化校验：1 个该情景订单组合×¥38,000=¥38,000；模型合计=¥38,000 | ¥38,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 8 天；含工时口径：按首月 18 小时、目标时薪 ¥200/小时，需覆盖约 ¥4,100，按保守情景折算约 62 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**先交一个产品的公开样板；3 个团队访谈中 1 个预付 ¥2,000 报告费
4. **技术验收门槛：**在约定样本/页面上复测“主题覆盖、异常提前量、人工确认率、客户采取行动数”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。
...
|---:|---|---|---|
| Day 1 | 定一条窄线 | Google Sheets > Blank > scope tab | 写唯一买方“社区银行、信用合作社、fintech 客服/产品/合规团队”、唯一数据主题和试点“一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要”；不先建泛平台 |
| Day 2 | 查许可 | 打开本文件全部官方来源 > Terms/API docs | 记录允许用途、署名/声明、限流、第三方权利和更新时间；许可不清时不收费、不处理真实数据、不交付，先取得书面分类 |
| Day 3 | 取 10–20 条官方样本 | CFPB Complaint Database > Download data/API；Sheets > Import；Looker > Create Report | 优先用官方 UI、Saved Search、CSV 或 Postman；保存筛选条件、日期、状态码/导出名和 official ID，不假设通用脚本支持该来源 |
...
| Day 5 | 做人工样本 | 抽查 30 条叙述和 issue 标签；与 CFPB 过滤器数量对账 | 人工筛 20 条并逐条回链；把“真有用/无关/不确定”作为黄金标签 |
| Day 6 | 写匹配规则 | Sheets > rules tab | 输入：产品、问题词典、公司、时间窗口、异常阈值、禁用个体字段；把必须条件、加分项、排除词和人工核验写成可见规则 |
| Day 7 | 保存官方筛选/请求 | CFPB Complaint Database > Download data/API；Sheets > Import；Looker > Create Report | 保存官方 Saved Search、导出参数或 Postman collection；POST body、分页、认证和限流按该来源官方文档逐项记录，不用一个通用命令冒充全覆盖 |
...
| Day 10 | 出首份样报 | Google Docs > New from blank | 只放 5–15 条最相关结果；页首写用途、来源和局限；结果目标：按产品、问题、时间和公司聚合，显示样本量、原始叙述片段和异常而不作裁定 |
...
| Day 13 | 发布固定价服务 | Upwork > Find Work > Your services > Create Project | Upwork Price 字段输入 US$290（报告统一按 US$1=¥7 折算约 ¥2,030；执行日以平台/银行实际换汇为准）；粘贴标题、样报、来源许可和免责声明，先卖人工报告，不卖未完成订阅软件 |
| Day 14 | 列 20 个买方 | Upwork > Find Work > Search jobs > 输入 CFPB complaint data analysis；Sheets > targets | 记录 company/source_url/why_fit/jurisdiction/entity_type；只用公司级通用入口，不抓个人邮箱或社媒数据 |
| Day 15 | 首批 10 个触达 | Upwork > 打开匹配职位 > Apply now；或通过司法辖区闸门后 Gmail > Compose | 逐封附 3 条公开样报；真实身份、实体地址、隐私告知和退订齐全，不用追踪像素 |
| Day 16 | 访谈并进入合法合同路径 | Calendly > Event types > 20 min；普通职位 > Apply now 后等待客户 Offer；仅已付款旧客户或主动从 Project Catalog 发消息的客户 > Propose new contract | 访谈 3 人：上次错过什么、哪些字段最值钱、谁批准；Upwork 总价输入 US$290（报告统一按 US$1=¥7 折算约 ¥2,030；执行日以平台/银行实际换汇为准）。普通职位客户必须由客户发送 Offer，卖方不能把 Propose new contract 当通用按钮；技术验收与付款分开 |
| Day 17 | 核验首个付费阶段已注资 | Upwork > Offers/Your active contracts > Fixed-price > 当前 milestone/Project Catalog order > status | 客户侧完成购买或注资；卖方只在当前段显示 Active/Funded 后接受/开工，绝不代客户点击 Fund。商业验证：先交一个产品的公开样板；3 个团队访谈中 1 个预付 ¥2,000 报告费；未注资只保留样报，不继续做软件或批量采集 |
| Day 18 | 重做筛选 | Sheets > rules tab > Duplicate v1 to v2 | 只改最大误报来源；用原 20 条黄金集重新算精确率/召回率并保留 v1 |
| Day 19 | 启动付费实时周 | 抽查 30 条叙述和 issue 标签；与 CFPB 过滤器数量对账 | 按官方更新频率手工/官方导出运行；每次记录筛选、响应/导出、条数、失败、版本和抓取日 |
...
| Day 22 | 确认下一段已注资再看使用 | Upwork > Contract > Milestones；Sheets > customer feedback tab | 自定义合同只在下一里程碑为 Active/Funded 后继续；询问哪些结果被打开/采取行动，只看 主题覆盖、异常提前量、人工确认率、客户采取行动数，不把外部成交全部归因给报告 |
...
> CFPB 投诉主题与异常看板｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向社区银行、信用合作社、fintech 客服/产品/合规团队。我会在不改变生产关键动作的前提下，完成「一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要」，并用 主题覆盖、异常提前量、人工确认率、客户采取行动数 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：公开投诉量大，团队难发现产品、问题与公司的主题变化。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：按产品、问题、时间和公司聚合，显示样本量、原始叙述片段和异常而不作裁定。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：¥2,000–8,000/报告；¥999–3,999/月。
...
**主题：**关于贵司「CFPB 投诉主题与异常看板」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：公开投诉量大，团队难发现产品、问题与公司的主题变化。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「一个产品线、过去 12 个月、一个 Looker 仪表盘和月度摘要」，验收只看 主题覆盖、异常提前量、人工确认率、客户采取行动数。  
...
> 补充一个具体点：本试点的最小通过条件是「先交一个产品的公开样板；3 个团队访谈中 1 个预付 ¥2,000 报告费」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
先按 `CFPB Complaint Database > Download data/API；Sheets > Import；Looker > Create Report` 建最小副本/测试环境，再按 `抽查 30 条叙述和 issue 标签；与 CFPB 过滤器数量对账` 跑 5 条正常样本、1 条空值、1 条重复、1 条权限拒绝和 1 条回滚。技术通过条件：在约定样本/页面上复测“主题覆盖、异常提前量、人工确认率、客户采取行动数”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。 任何涉及发送、付款、退款、删除、公开发布、法律/医疗/金融判断的动作保持人工批准。
...
- **风险：投诉被当作监管裁定**　应对：显著标注投诉未经证实、近期数据可能审核中
- **风险：消费者叙述再识别**　应对：只用官方已脱敏叙述，聚合展示并设置最小样本阈值
...
- **Go：**达到本方法的商业验证门槛：“先交一个产品的公开样板；3 个团队访谈中 1 个预付 ¥2,000 报告费”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
