# 方法 34｜TED 欧盟招标中文垂直简报

> **一页结论：**面向希望进入欧盟采购的中国工业设备、零部件和 B2B 服务出口商，用「一个 CPV/国家组合、7 天日报、最多 15 条机会」先收费验证。启动现金成本 ¥100–600，目标是 有效机会占比、原公告点击、销售确认有效数、节省小时。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 公共数据情报 |
| 买方 | 希望进入欧盟采购的中国工业设备、零部件和 B2B 服务出口商 |
| 当前痛点 | TED 多语公告复杂，企业难持续筛选 CPV、地区、金额和资格 |
| 可交付结果 | 一个 CPV 垂直的中文摘要、资格/截止/文件链接和 bid/no-bid 清单 |
| 最小试点 | 一个 CPV/国家组合、7 天日报、最多 15 条机会 |
| 工具栈 | TED Search API + feed_alert.py + Sheets + 人工翻译 QA |
| 启动成本 | ¥100–600（不含自己的人工） |
| 时间 | 7–14 天 |
| 技能 | TED/CPV、双语摘要、API、欧盟采购基础 |
| 参考测试报价 | ¥699–2,499/月；定制 ¥2,000–6,000 |
| 最小验证 | 给 5 家同垂直出口商发 3 条免费样报；至少 1 家预付 ¥999/月 |
| 综合分 | 4.15/5；需求证据 4 / 验证速度 3 / 低成本 4 / 复购性 5 / 自动化杠杆 5 / 获客可达 4 / 风险可控 4 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **TED 多语公告复杂，企业难持续筛选 CPV、地区、金额和资格** 变成一个买方能验收的固定范围结果：**一个 CPV 垂直的中文摘要、资格/截止/文件链接和 bid/no-bid 清单**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。

### 当前市场证据

- **D04｜[TED Search API](https://docs.ted.europa.eu/api/latest/search.html)：**TED Search API v3 支持匿名 POST 查询、字段选择、PAGE_NUMBER/ITERATION 分页和 onlyLatestVersions；响应提供官方 links.html 语言回链。 **使用边界：**PAGE_NUMBER 每页最多 250、最多 15,000 条；超过应使用 ITERATION token。保留来源、语言和提取日期，不镜像权利不明附件。
- **D25｜[TED Search API pagination and expert search](https://docs.ted.europa.eu/ODS/latest/reuse/search-api.html)：**官方分页说明区分 PAGE_NUMBER 与 ITERATION，ITERATION 使用 iterationNextToken；expert search 语法支持字段条件组合。 **使用边界：**checkQuerySyntax=true 只用于语法检查；真正取数改回 false，并优先保存响应中的 links.html 官方回链。
...
泛招标数据库已有规模；差异必须是一个 CPV、中文、资格解释和人工核验。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 D04、D25、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**TED 欧盟招标中文垂直简报 30 天验证包
- **交付：**一个 CPV/国家组合、7 天日报、最多 15 条机会；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**CPV、国家、合同金额、语言、资格、排除词、公司能力
- **验收指标：**有效机会占比、原公告点击、销售确认有效数、节省小时
- **参考报价：**¥699–2,499/月；定制 ¥2,000–6,000
...
| 中性 | 中性 10 个 ¥999 订阅；归一化校验：1 个该情景订单组合×¥9,990=¥9,990；模型合计=¥9,990 | ¥9,990 |
| 乐观 | 乐观 20 个订阅加 4 份定制；归一化校验：1 个该情景订单组合×¥35,000=¥35,000；模型合计=¥35,000 | ¥35,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 19 天；含工时口径：按首月 18 小时、目标时薪 ¥200/小时，需覆盖约 ¥4,200，按保守情景折算约 127 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「一个 CPV/国家组合、7 天日报、最多 15 条机会」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**给 5 家同垂直出口商发 3 条免费样报；至少 1 家预付 ¥999/月
4. **技术验收门槛：**在约定样本/页面上复测“有效机会占比、原公告点击、销售确认有效数、节省小时”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。
...
|---:|---|---|---|
| Day 1 | 定一条窄线 | Google Sheets > Blank > scope tab | 写唯一买方“希望进入欧盟采购的中国工业设备、零部件和 B2B 服务出口商”、唯一数据主题和试点“一个 CPV/国家组合、7 天日报、最多 15 条机会”；不先建泛平台 |
| Day 2 | 查许可 | 打开本文件全部官方来源 > Terms/API docs | 记录允许用途、署名/声明、限流、第三方权利和更新时间；许可不清时不收费、不处理真实数据、不交付，先取得书面分类 |
| Day 3 | 跑官方精确请求 | Postman > New > HTTP Request > POST > https://api.ted.europa.eu/v3/notices/search > Body > raw > JSON | 粘贴本文件 TED JSON，先可用 checkQuerySyntax=true 查语法，再改 false 取数；抽查 links.html.ENG 与双语字段 |
...
| Day 5 | 做人工样本 | TED > Search notices > 用 notice identifier 回查；Sheets > 双语抽检 | 人工筛 20 条并逐条回链；把“真有用/无关/不确定”作为黄金标签 |
| Day 6 | 写匹配规则 | Sheets > rules tab | 输入：CPV、国家、合同金额、语言、资格、排除词、公司能力；把必须条件、加分项、排除词和人工核验写成可见规则 |
| Day 7 | 保存分页/增量契约 | Postman > Body > paginationMode/page/iterationNextToken；Response > links.html | PAGE_NUMBER 增 page；超过 15,000 改 ITERATION 并传 token。只保存响应官方语言回链，不拼第三方 URL |
...
| Day 10 | 出首份样报 | Google Docs > New from blank | 只放 5–15 条最相关结果；页首写用途、来源和局限；结果目标：一个 CPV 垂直的中文摘要、资格/截止/文件链接和 bid/no-bid 清单 |
...
| Day 14 | 列 20 个买方 | Upwork > Find Work > Search jobs > 输入 EU tender research Chinese；Sheets > targets | 记录 company/source_url/why_fit/jurisdiction/entity_type；只用公司级通用入口，不抓个人邮箱或社媒数据 |
...
| Day 17 | 核验首个付费阶段已注资 | Upwork > Offers/Your active contracts > Fixed-price > 当前 milestone/Project Catalog order > status | 客户侧完成购买或注资；卖方只在当前段显示 Active/Funded 后接受/开工，绝不代客户点击 Fund。商业验证：给 5 家同垂直出口商发 3 条免费样报；至少 1 家预付 ¥999/月；未注资只保留样报，不继续做软件或批量采集 |
| Day 18 | 重做筛选 | Sheets > rules tab > Duplicate v1 to v2 | 只改最大误报来源；用原 20 条黄金集重新算精确率/召回率并保留 v1 |
| Day 19 | 启动付费实时周 | TED > Search notices > 用 notice identifier 回查；Sheets > 双语抽检 | 按官方更新频率手工/官方导出运行；每次记录筛选、响应/导出、条数、失败、版本和抓取日 |
...
| Day 22 | 确认下一段已注资再看使用 | Upwork > Contract > Milestones；Sheets > customer feedback tab | 自定义合同只在下一里程碑为 Active/Funded 后继续；询问哪些结果被打开/采取行动，只看 有效机会占比、原公告点击、销售确认有效数、节省小时，不把外部成交全部归因给报告 |
...
> TED 欧盟招标中文垂直简报｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向希望进入欧盟采购的中国工业设备、零部件和 B2B 服务出口商。我会在不改变生产关键动作的前提下，完成「一个 CPV/国家组合、7 天日报、最多 15 条机会」，并用 有效机会占比、原公告点击、销售确认有效数、节省小时 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：TED 多语公告复杂，企业难持续筛选 CPV、地区、金额和资格。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：一个 CPV 垂直的中文摘要、资格/截止/文件链接和 bid/no-bid 清单。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：¥699–2,499/月；定制 ¥2,000–6,000。
...
**主题：**关于贵司「TED 欧盟招标中文垂直简报」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：TED 多语公告复杂，企业难持续筛选 CPV、地区、金额和资格。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「一个 CPV/国家组合、7 天日报、最多 15 条机会」，验收只看 有效机会占比、原公告点击、销售确认有效数、节省小时。  
...
> 补充一个具体点：本试点的最小通过条件是「给 5 家同垂直出口商发 3 条免费样报；至少 1 家预付 ¥999/月」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
- **风险：翻译改变法律含义**　应对：关键资格保留原文和官方链接，明确中文仅辅助理解
- **风险：数据/附件权利**　应对：注明来源、抓取日期和语言，不镜像第三方附件
...
- **Go：**达到本方法的商业验证门槛：“给 5 家同垂直出口商发 3 条免费样报；至少 1 家预付 ¥999/月”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
