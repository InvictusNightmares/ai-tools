# 方法 37｜Companies House 新注册企业 SIC 垂直线索

> **一页结论：**面向服务英国新公司的会计、保险、办公和网站服务商，用「一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像」先收费验证。启动现金成本 ¥0–300，目标是 公司有效率、行业准确率、客户接受率、退订/投诉。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 公共数据情报 |
| 买方 | 服务英国新公司的会计、保险、办公和网站服务商 |
| 当前痛点 | 泛线索库过时，难在公司成立后及时发现细分企业 |
| 可交付结果 | 按 SIC、地区和成立日期筛公司级记录，保留官方状态和网站研究字段 |
| 最小试点 | 一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像 |
| 工具栈 | Companies House API/data product + Sheets + 人工官网核验 |
| 启动成本 | ¥0–300（不含自己的人工） |
| 时间 | 5–10 天 |
| 技能 | API、SIC、公司研究、GDPR/PECR 边界 |
| 参考测试报价 | ¥499–1,499/月；定制 ¥1,000–3,000 |
| 最小验证 | 50 条样本公司级准确率至少 95%；找 3 个服务商，1 个预付 ¥699/月 |
| 综合分 | 4.30/5；需求证据 4 / 验证速度 4 / 低成本 5 / 复购性 5 / 自动化杠杆 5 / 获客可达 4 / 风险可控 3 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **泛线索库过时，难在公司成立后及时发现细分企业** 变成一个买方能验收的固定范围结果：**按 SIC、地区和成立日期筛公司级记录，保留官方状态和网站研究字段**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。

### 当前市场证据

- **D08｜[Companies House API Get Started](https://developer.company-information.service.gov.uk/get-started)：**英国 Companies House 开发者入门页提供公司资料 REST API 的认证和调用入口。 **使用边界：**该链接只支持 API/认证事实；限额、免费快照和资料未验证需分别看 D21–D23。
- **D21｜[Companies House developer guidelines](https://developer.company-information.service.gov.uk/developer-guidelines/)：**Companies House 开发者指南列出默认 API 限额为每 5 分钟 600 次请求。 **使用边界：**限额可变；必须做低频、缓存、指数退避，不得为绕限额轮换密钥。
- **D22｜[Companies House data products](https://www.gov.uk/guidance/companies-house-data-products)：**Companies House 提供免费的 live companies 月度 CSV 快照；数据编制截至上月末，并通常在月末后 5 个工作日内发布。 **使用边界：**月度快照不是实时成立公司 feed；使用者仍须自行核验数据保护、版权与适用法律，本方案的 14 天样本优先用官方搜索/API并标取得日。
- **D23｜[Companies House disclaimer](https://resources.companieshouse.gov.uk/disclaimer.shtml)：**Companies House 声明对提交信息不作全面验证，用户不应把登记资料当成已核实商业事实。 **使用边界：**每条交付必须保留官方记录链接/查询日并标“未独立验证”；不得以此推断信用、营业或联系同意。
...
- **C03｜[ICO Business-to-business marketing](https://ico.org.uk/for-organisations/direct-marketing-and-privacy-and-electronic-communications/business-to-business-marketing/)：**英国 PECR 对 corporate subscriber 的 B2B 邮件通常不要求事先同意，但必须标识发送者并提供有效退订；sole trader 和部分 partnership 通常按 individual subscriber 处理。 **使用边界：**个人化工作邮箱仍可能受 UK GDPR 约束；公开数据不是营销同意。收件人类型不明时按个人处理，并在发送前复核正在更新的 ICO 指南。
...
公司数据平台更全面；你的切口是一个 SIC、成立后 14 天和人工业务相关性。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 D08、D21、D22、D23、C01、C02、C03、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**Companies House 新注册企业 SIC 垂直线索 30 天验证包
- **交付：**一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**SIC、地区、成立窗口、排除状态、公司级相关性规则
- **验收指标：**公司有效率、行业准确率、客户接受率、退订/投诉
- **参考报价：**¥499–1,499/月；定制 ¥1,000–3,000

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 个预售；归一化校验：1 个该情景订单组合×¥699=¥699；模型合计=¥699 | ¥699 |
| 中性 | 中性 10 个 ¥699 订阅；归一化校验：1 个该情景订单组合×¥6,990=¥6,990；模型合计=¥6,990 | ¥6,990 |
| 乐观 | 乐观 20 个订阅加 4 个定制清单；归一化校验：1 个该情景订单组合×¥24,000=¥24,000；模型合计=¥24,000 | ¥24,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 13 天；含工时口径：按首月 18 小时、目标时薪 ¥200/小时，需覆盖约 ¥3,900，按保守情景折算约 168 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**50 条样本公司级准确率至少 95%；找 3 个服务商，1 个预付 ¥699/月
4. **技术验收门槛：**在约定样本/页面上复测“公司有效率、行业准确率、客户接受率、退订/投诉”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。
...
|---:|---|---|---|
| Day 1 | 定一条窄线 | Google Sheets > Blank > scope tab | 写唯一买方“服务英国新公司的会计、保险、办公和网站服务商”、唯一数据主题和试点“一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像”；不先建泛平台 |
| Day 2 | 查许可 | 打开本文件全部官方来源 > Terms/API docs | 记录允许用途、署名/声明、限流、第三方权利和更新时间；许可不清时不收费、不处理真实数据、不交付，先取得书面分类 |
| Day 3 | 取 10–20 条官方样本 | Companies House > Find company information > Advanced company search；先用 company number/成立日/SIC 手工取样，API 只在客户付款后启用 | 优先用官方 UI、Saved Search、CSV 或 Postman；保存筛选条件、日期、状态码/导出名和 official ID，不假设通用脚本支持该来源 |
...
| Day 5 | 做人工样本 | Companies House > Find company information > 用 company number 回查 10 条 | 人工筛 20 条并逐条回链；把“真有用/无关/不确定”作为黄金标签 |
| Day 6 | 写匹配规则 | Sheets > rules tab | 输入：SIC、地区、成立窗口、排除状态、公司级相关性规则；把必须条件、加分项、排除词和人工核验写成可见规则 |
| Day 7 | 保存官方筛选/请求 | Companies House > Find company information > Advanced company search；先用 company number/成立日/SIC 手工取样，API 只在客户付款后启用 | 保存官方 Saved Search、导出参数或 Postman collection；POST body、分页、认证和限流按该来源官方文档逐项记录，不用一个通用命令冒充全覆盖 |
...
| Day 10 | 出首份样报 | Google Docs > New from blank | 只放 5–15 条最相关结果；页首写用途、来源和局限；结果目标：按 SIC、地区和成立日期筛公司级记录，保留官方状态和网站研究字段 |
...
| Day 14 | 列 20 个买方 | Upwork > Find Work > Search jobs > 输入 Companies House lead research；Sheets > targets | 记录 company/source_url/why_fit/jurisdiction/entity_type；只用公司级通用入口，不抓个人邮箱或社媒数据 |
...
| Day 17 | 核验首个付费阶段已注资 | Upwork > Offers/Your active contracts > Fixed-price > 当前 milestone/Project Catalog order > status | 客户侧完成购买或注资；卖方只在当前段显示 Active/Funded 后接受/开工，绝不代客户点击 Fund。商业验证：50 条样本公司级准确率至少 95%；找 3 个服务商，1 个预付 ¥699/月；未注资只保留样报，不继续做软件或批量采集 |
| Day 18 | 重做筛选 | Sheets > rules tab > Duplicate v1 to v2 | 只改最大误报来源；用原 20 条黄金集重新算精确率/召回率并保留 v1 |
| Day 19 | 启动付费实时周 | Companies House > Find company information > 用 company number 回查 10 条 | 按官方更新频率手工/官方导出运行；每次记录筛选、响应/导出、条数、失败、版本和抓取日 |
...
| Day 22 | 确认下一段已注资再看使用 | Upwork > Contract > Milestones；Sheets > customer feedback tab | 自定义合同只在下一里程碑为 Active/Funded 后继续；询问哪些结果被打开/采取行动，只看 公司有效率、行业准确率、客户接受率、退订/投诉，不把外部成交全部归因给报告 |
...
> Companies House 新注册企业 SIC 垂直线索｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向服务英国新公司的会计、保险、办公和网站服务商。我会在不改变生产关键动作的前提下，完成「一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像」，并用 公司有效率、行业准确率、客户接受率、退订/投诉 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：泛线索库过时，难在公司成立后及时发现细分企业。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：按 SIC、地区和成立日期筛公司级记录，保留官方状态和网站研究字段。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：¥499–1,499/月；定制 ¥1,000–3,000。
...
**主题：**关于贵司「Companies House 新注册企业 SIC 垂直线索」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：泛线索库过时，难在公司成立后及时发现细分企业。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「一个 SIC/地区、过去 14 天、50 家公司级清单，不含个人画像」，验收只看 公司有效率、行业准确率、客户接受率、退订/投诉。  
...
> 补充一个具体点：本试点的最小通过条件是「50 条样本公司级准确率至少 95%；找 3 个服务商，1 个预付 ¥699/月」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
先按 `Companies House > Find company information > Advanced company search；先用 company number/成立日/SIC 手工取样，API 只在客户付款后启用` 建最小副本/测试环境，再按 `Companies House > Find company information > 用 company number 回查 10 条` 跑 5 条正常样本、1 条空值、1 条重复、1 条权限拒绝和 1 条回滚。技术通过条件：在约定样本/页面上复测“公司有效率、行业准确率、客户接受率、退订/投诉”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。 任何涉及发送、付款、退款、删除、公开发布、法律/医疗/金融判断的动作保持人工批准。
...
- **风险：公开数据被当作已验证事实**　应对：显著说明 Companies House 不验证准确性并回链官方记录
- **风险：董事个人画像或违规外联**　应对：默认只交公司字段，不拼接董事、个人邮箱或 LinkedIn 数据；收件人类型不明时按 individual subscriber 处理
...
- **Go：**达到本方法的商业验证门槛：“50 条样本公司级准确率至少 95%；找 3 个服务商，1 个预付 ¥699/月”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
