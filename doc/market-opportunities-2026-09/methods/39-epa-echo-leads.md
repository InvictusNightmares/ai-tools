# 方法 39｜EPA ECHO DC 活跃重大设施检查/行动/罚款信号

> **一页结论：**面向环保顾问、检测实验室、废水/过滤设备商和 EHS 软件公司，用「DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号」先收费验证。启动现金成本 ¥100–500，目标是 RegistryID/官方回链准确率、三类事件日期完整率、人工核验率、销售接受率。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 公共数据情报 |
| 买方 | 环保顾问、检测实验室、废水/过滤设备商和 EHS 软件公司 |
| 当前痛点 | 难以及时发现 DC 活跃重大设施最近出现的检查、正式行动或罚款日期信号 |
| 可交付结果 | 针对 DC 活跃重大设施，只读取最近检查、最近正式行动和最近罚款三个日期字段，按过去 30 天本地过滤，交付公司级候选信号、官方链接和非定罪措辞 |
| 最小试点 | DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号 |
| 工具栈 | EPA ECHO API/Exporter + Sheets + Looker Studio |
| 启动成本 | ¥100–500（不含自己的人工） |
| 时间 | 10–21 天 |
| 技能 | ECHO 数据、设施实体、谨慎事实表述、B2B 研究 |
| 参考测试报价 | ¥999–3,999/月；定制报告 ¥2,000–6,000 |
| 最小验证 | 给 3 个环保垂直服务商展示 10 条免费样报；至少 1 个为 DC 30 天报告预付 ¥1,200 |
| 综合分 | 4.00/5；需求证据 4 / 验证速度 3 / 低成本 5 / 复购性 5 / 自动化杠杆 5 / 获客可达 3 / 风险可控 3 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **难以及时发现 DC 活跃重大设施最近出现的检查、正式行动或罚款日期信号** 变成一个买方能验收的固定范围结果：**针对 DC 活跃重大设施，只读取最近检查、最近正式行动和最近罚款三个日期字段，按过去 30 天本地过滤，交付公司级候选信号、官方链接和非定罪措辞**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。

### 当前市场证据

- **D12｜[EPA ECHO web services](https://echo.epa.gov/tools/web-services)：**ECHO facilities 查询先返回 QueryID，再用 get_qid 分页取得 Facilities；RegistryID 才是稳定设施标识，可回链 Detailed Facility Report。当前窄范围只使用 FacDateLastInspection、FacDateLastFormalAction、FacDateLastPenalty。 **使用边界：**QueryID 只是查询会话，不能当 source ID；三个日期字段可能滞后，须按 [as_of-29d, as_of] 本地过滤，并以 RegistryID|event_type|event_date 去重；不能把日期信号表述为当前违法。
...
环境情报供应商面向企业；机会是一个州/设备场景的轻量信号。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 D12、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**EPA ECHO DC 活跃重大设施检查/行动/罚款信号 30 天验证包
- **交付：**DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**固定地区 DC、active=Y、major=Y、30 天 as_of 窗口、检查/正式行动/罚款三类关键词与排除词
- **验收指标：**RegistryID/官方回链准确率、三类事件日期完整率、人工核验率、销售接受率
- **参考报价：**¥999–3,999/月；定制报告 ¥2,000–6,000

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 个预售；归一化校验：1 个该情景订单组合×¥1,200=¥1,200；模型合计=¥1,200 | ¥1,200 |
| 中性 | 中性 8 个订阅；归一化校验：1 个该情景订单组合×¥10,000=¥10,000；模型合计=¥10,000 | ¥10,000 |
| 乐观 | 乐观 20 个订阅加 3 份定制；归一化校验：1 个该情景订单组合×¥34,000=¥34,000；模型合计=¥34,000 | ¥34,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 13 天；含工时口径：按首月 18 小时、目标时薪 ¥200/小时，需覆盖约 ¥4,100，按保守情景折算约 103 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**给 3 个环保垂直服务商展示 10 条免费样报；至少 1 个为 DC 30 天报告预付 ¥1,200
4. **技术验收门槛：**在约定样本/页面上复测“RegistryID/官方回链准确率、三类事件日期完整率、人工核验率、销售接受率”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。
...
|---:|---|---|---|
| Day 1 | 定一条窄线 | Google Sheets > Blank > scope tab | 写唯一买方“环保顾问、检测实验室、废水/过滤设备商和 EHS 软件公司”、唯一数据主题和试点“DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号”；不先建泛平台 |
| Day 2 | 查许可 | 打开本文件全部官方来源 > Terms/API docs | 记录允许用途、署名/声明、限流、第三方权利和更新时间；许可不清时不收费、不处理真实数据、不交付，先取得书面分类 |
| Day 3 | 跑官方精确请求 | Postman > New > GET > 本文件 ECHO get_facilities URL；Response > Results.QueryID | 复制 QueryID 到 get_qid URL，按 pageno 拉取；稳定 source_id 用 RegistryID，并打开 Detailed Facility Report 回查；只读取 FacDateLastInspection/FacDateLastFormalAction/FacDateLastPenalty |
...
| Day 5 | 做人工样本 | ECHO Facility Search > 用 RegistryID 回查 10 条；逐条核 FacDateLastInspection/FacDateLastFormalAction/FacDateLastPenalty 与更新时间 | 人工筛 20 条并逐条回链；把“真有用/无关/不确定”作为黄金标签 |
| Day 6 | 写匹配规则 | Sheets > rules tab | 输入：固定地区 DC、active=Y、major=Y、30 天 as_of 窗口、检查/正式行动/罚款三类关键词与排除词；把必须条件、加分项、排除词和人工核验写成可见规则 |
| Day 7 | 保存分页/增量契约 | Postman > get_qid > pageno；Sheets > events/date-filter | 到 Facilities 为空/少于 responseset 或达到 QueryRows；按 [as_of-29d, as_of] 过滤，以 RegistryID\|event_type\|event_date 去重；窗口内不足 20 条或为 0 条都如实报告 |
...
| Day 10 | 出首份样报 | Google Docs > New from blank | 只放 5–15 条最相关结果；页首写用途、来源和局限；结果目标：针对 DC 活跃重大设施，只读取最近检查、最近正式行动和最近罚款三个日期字段，按过去 30 天本地过滤，交付公司级候选信号、官方链接和非定罪措辞 |
...
| Day 13 | 发布固定价服务 | Upwork > Find Work > Your services > Create Project | Upwork Price 字段输入 US$175（报告统一按 US$1=¥7 折算约 ¥1,225；执行日以平台/银行实际换汇为准）；粘贴标题、样报、来源许可和免责声明，先卖人工报告，不卖未完成订阅软件 |
| Day 14 | 列 20 个买方 | Upwork > Find Work > Search jobs > 输入 EPA ECHO research；Sheets > targets | 记录 company/source_url/why_fit/jurisdiction/entity_type；只用公司级通用入口，不抓个人邮箱或社媒数据 |
| Day 15 | 首批 10 个触达 | Upwork > 打开匹配职位 > Apply now；或通过司法辖区闸门后 Gmail > Compose | 逐封附 3 条公开样报；真实身份、实体地址、隐私告知和退订齐全，不用追踪像素 |
| Day 16 | 访谈并进入合法合同路径 | Calendly > Event types > 20 min；普通职位 > Apply now 后等待客户 Offer；仅已付款旧客户或主动从 Project Catalog 发消息的客户 > Propose new contract | 访谈 3 人：上次错过什么、哪些字段最值钱、谁批准；Upwork 总价输入 US$175（报告统一按 US$1=¥7 折算约 ¥1,225；执行日以平台/银行实际换汇为准）。普通职位客户必须由客户发送 Offer，卖方不能把 Propose new contract 当通用按钮；技术验收与付款分开 |
| Day 17 | 核验首个付费阶段已注资 | Upwork > Offers/Your active contracts > Fixed-price > 当前 milestone/Project Catalog order > status | 客户侧完成购买或注资；卖方只在当前段显示 Active/Funded 后接受/开工，绝不代客户点击 Fund。商业验证：给 3 个环保垂直服务商展示 10 条免费样报；至少 1 个为 DC 30 天报告预付 ¥1,200；未注资只保留样报，不继续做软件或批量采集 |
| Day 18 | 重做筛选 | Sheets > rules tab > Duplicate v1 to v2 | 只改最大误报来源；用原 20 条黄金集重新算精确率/召回率并保留 v1 |
| Day 19 | 启动付费实时周 | ECHO Facility Search > 用 RegistryID 回查 10 条；逐条核 FacDateLastInspection/FacDateLastFormalAction/FacDateLastPenalty 与更新时间 | 按官方更新频率手工/官方导出运行；每次记录筛选、响应/导出、条数、失败、版本和抓取日 |
...
| Day 22 | 确认下一段已注资再看使用 | Upwork > Contract > Milestones；Sheets > customer feedback tab | 自定义合同只在下一里程碑为 Active/Funded 后继续；询问哪些结果被打开/采取行动，只看 RegistryID/官方回链准确率、三类事件日期完整率、人工核验率、销售接受率，不把外部成交全部归因给报告 |
...
> EPA ECHO DC 活跃重大设施检查/行动/罚款信号｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向环保顾问、检测实验室、废水/过滤设备商和 EHS 软件公司。我会在不改变生产关键动作的前提下，完成「DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号」，并用 RegistryID/官方回链准确率、三类事件日期完整率、人工核验率、销售接受率 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：难以及时发现 DC 活跃重大设施最近出现的检查、正式行动或罚款日期信号。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：针对 DC 活跃重大设施，只读取最近检查、最近正式行动和最近罚款三个日期字段，按过去 30 天本地过滤，交付公司级候选信号、官方链接和非定罪措辞。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：¥999–3,999/月；定制报告 ¥2,000–6,000。
...
**主题：**关于贵司「EPA ECHO DC 活跃重大设施检查/行动/罚款信号」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：难以及时发现 DC 活跃重大设施最近出现的检查、正式行动或罚款日期信号。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「DC 活跃重大设施、过去 30 天、最多 20 条经人工核验的检查/正式行动/罚款日期信号」，验收只看 RegistryID/官方回链准确率、三类事件日期完整率、人工核验率、销售接受率。  
...
> 补充一个具体点：本试点的最小通过条件是「给 3 个环保垂直服务商展示 10 条免费样报；至少 1 个为 DC 30 天报告预付 ¥1,200」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
- **风险：历史记录被写成当前违法**　应对：使用状态、日期和中性措辞，禁止定罪式销售文案
- **风险：数据滞后**　应对：显示源更新时间，关键动作前要求客户回查官方记录
...
- **Go：**达到本方法的商业验证门槛：“给 3 个环保垂直服务商展示 10 条免费样报；至少 1 个为 DC 30 天报告预付 ¥1,200”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
