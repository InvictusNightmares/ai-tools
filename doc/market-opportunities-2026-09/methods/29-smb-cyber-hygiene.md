...
> **一页结论：**面向没有安全负责人、依赖 SaaS 和云盘的 5–50 人公司，用「一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试」先收费验证。启动现金成本 ¥0–300，目标是 MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 网站技术服务 |
| 买方 | 没有安全负责人、依赖 SaaS 和云盘的 5–50 人公司 |
| 当前痛点 | 没有 MFA、共享管理员、备份未测试、离职账号未关和日志空白 |
| 可交付结果 | 对一个 Google Workspace 或 Microsoft 365 套件和客户已有备份系统执行 10 项有证据的只读检查；客户批准后完成最多 3 个低风险修复和 1 次隔离恢复 |
| 最小试点 | 一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试 |
| 工具栈 | CISA 清单 + Google/Microsoft Admin + 客户备份工具 |
| 启动成本 | ¥0–300（不含自己的人工） |
| 时间 | 5–10 天 |
| 技能 | 身份管理、备份、SaaS 管理、风险沟通 |
| 参考测试报价 | 基线 ¥4,000–12,000；季度复检 ¥1,500–4,000 |
| 最小验证 | 1 个客户为 10 项基线试点注资/支付 ¥4,000 |
| 综合分 | 4.45/5；需求证据 5 / 验证速度 4 / 低成本 5 / 复购性 4 / 自动化杠杆 4 / 获客可达 5 / 风险可控 4 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **没有 MFA、共享管理员、备份未测试、离职账号未关和日志空白** 变成一个买方能验收的固定范围结果：**对一个 Google Workspace 或 Microsoft 365 套件和客户已有备份系统执行 10 项有证据的只读检查；客户批准后完成最多 3 个低风险修复和 1 次隔离恢复**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。
...
- **M24｜[CISA Small and Medium Business Resources](https://www.cisa.gov/small-and-medium-sized-business-resources)：**CISA 面向中小企业列出网络钓鱼、密码、MFA、更新、日志、备份和加密等核心实践并提供免费资源。 **使用边界：**本方案是基础卫生检查，不是渗透测试、认证或零风险保证。
...
MSP 和安全顾问更全面；你的范围是可见、低风险、可验证的基础卫生。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 M01、M24、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**中小企业网络安全卫生基线 30 天验证包
- **交付：**一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**资产清单、管理员、离职流程、备份位置、恢复目标、紧急联系人
- **验收指标：**MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟
- **参考报价：**基线 ¥4,000–12,000；季度复检 ¥1,500–4,000

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 个基线；归一化校验：1 个该情景订单组合×¥4,000=¥4,000；模型合计=¥4,000 | ¥4,000 |
| 中性 | 中性 3 个基线加 2 个复检；归一化校验：1 个该情景订单组合×¥16,000=¥16,000；模型合计=¥16,000 | ¥16,000 |
| 乐观 | 乐观 7 个基线加 6 个复检；归一化校验：1 个该情景订单组合×¥43,000=¥43,000；模型合计=¥43,000 | ¥43,000 |
...
1. 不先做完整产品；只做「一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**1 个客户为 10 项基线试点注资/支付 ¥4,000
4. **技术验收门槛：**10/10 项都有时间戳、来源、状态、owner 和整改建议；3 个批准修复都有前后证据与回滚；1 个非敏感测试文件在隔离位置恢复成功且生产未覆盖
...
|---:|---|---|---|
| Day 1 | 定边界 | Google Sheets > Blank spreadsheet；建 scope、baseline、risk 三个 tab | 写入买方“没有安全负责人、依赖 SaaS 和云盘的 5–50 人公司”、固定范围“一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试”；产出一页范围，禁止扩到高风险动作 |
| Day 2 | 核证据 | 打开本文件“市场证据”中的全部官方链接；浏览器 > Bookmark folder | 逐条记录发布日期、事实与局限；如果关键链接失效，暂停宣传该事实 |
| Day 3 | 建 30 个 ICP | Upwork > Find Work > Search jobs > 输入 Google Workspace security audit；公司官网只看 Contact/Team 通用入口 | Sheets targets 列 company/source_url/why_fit/jurisdiction/entity_type/status；只录与“没有 MFA、共享管理员、备份未测试、离职账号未关和日志空白”直接相关的 30 个主体，不抓个人数据 |
| Day 4 | 量基线 | Google Sheets > baseline tab | 输入最近 30 天 MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟；没有数字就记录样本量、当前耗时和错误例子 |
| Day 5 | 开最小工具 | Google：Admin console > Directory > Users；Account > Admin roles；Security > Authentication > 2-Step Verification；Reporting > Audit and investigation > Admin log events。Microsoft：Entra admin center > Entra ID > Users；Roles & admins；Authentication methods > Activity；Monitoring & health > Audit logs/Sign-in logs | 只开试点所需功能；栈：CISA 清单 + Google/Microsoft Admin + 客户备份工具；保存账号 owner、权限、关闭/回滚路径截图 |
| Day 6 | 收脱敏样本 | Google Drive > New > Folder > Share > Restricted | 向测试客户索取：资产清单、管理员、离职流程、备份位置、恢复目标、紧急联系人；密码/API key 不放文档 |
...
| Day 12 | 做第一版规则 | 客户选定备份供应商官方文档 > Restore > 隔离文件夹/测试用户；路径未明确时只交只读证据，不覆盖生产 | 先只处理 5 条/1 页/1 个对象；字段来自“资产清单、管理员、离职流程、备份位置、恢复目标、紧急联系人”；保存 expected/actual/evidence，不做未经批准的生产写入 |
| Day 13 | 建立确定性实施表 | Google：Admin console > Directory > Users；Account > Admin roles；Security > Authentication > 2-Step Verification；Reporting > Audit and investigation > Admin log events。Microsoft：Entra admin center > Entra ID > Users；Roles & admins；Authentication methods > Activity；Monitoring & health > Audit logs/Sign-in logs | 把 资产清单、管理员、离职流程、备份位置、恢复目标、紧急联系人 拆成字段、确定性规则、owner、证据和回滚列；只实现合同明确要求的步骤 |
| Day 14 | 做反例与回滚测试 | 客户选定备份供应商官方文档 > Restore > 隔离文件夹/测试用户；路径未明确时只交只读证据，不覆盖生产 | 测试空值、重复、冲突、权限不足、断网和回滚；每例写预期/实际/证据，任何生产写入须客户逐项批准 |
| Day 15 | 加审计日志 | Google Sheets > logs tab；目标工具 > 活动/错误/运行历史 | 记录 event_id、时间、输入 hash、规则/配置版本、动作、审批人、错误和回滚；不记录无关 PII 或密钥 |
| Day 16 | 回放约定样本 | 客户选定备份供应商官方文档 > Restore > 隔离文件夹/测试用户；路径未明确时只交只读证据，不覆盖生产 | 执行技术验收：10/10 项都有时间戳、来源、状态、owner 和整改建议；3 个批准修复都有前后证据与回滚；1 个非敏感测试文件在隔离位置恢复成功且生产未覆盖；另记录商业验证阈值：1 个客户为 10 项基线试点注资/支付 ¥4,000 |
...
| Day 19 | 录 90 秒证据演示 | QuickTime Player > File > New Screen Recording；或 OBS > Start Recording | 按问题20秒→一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试30秒→测试30秒→边界10秒；遮住账号、密钥和客户数据 |
...
| Day 22 | 确认执行范围未变 | Google Docs > Proposal > Version history；Upwork > Contract > Milestones | 对照 Day 10 已注资合同，确认“一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试”、技术验收、权限、日期和停止条件未变；不重新谈或重复收试点 |
| Day 23 | 运行已注资阶段 | 客户选定备份供应商官方文档 > Restore > 隔离文件夹/测试用户；路径未明确时只交只读证据，不覆盖生产 | 自定义合同只在下一里程碑已注资后运行；Project Catalog 只在整单已注资后运行。按客户批准的最小权限先 5、再 20、再到约定上限；高风险错误、真实扣款或不可回滚变更立即停 |
| Day 24 | 每日 QA | Sheets > logs/QA tab > Create a filter | 每天抽查至少 10 条或全部小样本；只计算 MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟，保留分母、失败和不能归因部分 |
| Day 25 | 读结果 | Sheets > baseline vs pilot；Looker Studio 可选 | 只对比 MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟；写样本量和不能归因的部分 |
| Day 26 | 修一次 | CISA 清单 + Google/Microsoft Admin + 客户备份工具 > Duplicate/Clone/Version；仅在测试或副本中改 | 只修最大的一类错误；保留 v1/v2、变更说明、回放结果和恢复点，不同时改多个变量 |
...
> 中小企业网络安全卫生基线｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向没有安全负责人、依赖 SaaS 和云盘的 5–50 人公司。我会在不改变生产关键动作的前提下，完成「一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试」，并用 MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：没有 MFA、共享管理员、备份未测试、离职账号未关和日志空白。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：对一个 Google Workspace 或 Microsoft 365 套件和客户已有备份系统执行 10 项有证据的只读检查；客户批准后完成最多 3 个低风险修复和 1 次隔离恢复。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：基线 ¥4,000–12,000；季度复检 ¥1,500–4,000。
...
**主题：**关于贵司「中小企业网络安全卫生基线」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：没有 MFA、共享管理员、备份未测试、离职账号未关和日志空白。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试」，验收只看 MFA 覆盖、管理员数量、过期账号、备份恢复成功、关键更新延迟。  
...
> 补充一个具体点：本试点的最小通过条件是「1 个客户为 10 项基线试点注资/支付 ¥4,000」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
> **项目：**中小企业网络安全卫生基线 30 天验证  
> **客户：**{客户名}  
> **范围：**一个 Google Workspace 或 Microsoft 365 租户、一个客户已有备份系统；10 项检查、红黄绿报告、最多 3 项批准的低风险修复和 1 次隔离恢复，不做渗透测试  
> **客户提供：**资产清单、管理员、离职流程、备份位置、恢复目标、紧急联系人  
> **交付：**基线表、实施/配置、测试证据、异常队列、SOP、回滚说明、结果复盘  
> **技术验收：**10/10 项都有时间戳、来源、状态、owner 和整改建议；3 个批准修复都有前后证据与回滚；1 个非敏感测试文件在隔离位置恢复成功且生产未覆盖  
> **商业验证：**1 个客户为 10 项基线试点注资/支付 ¥4,000  
> **不包含：**未授权数据、法律/医疗/金融意见、批量群发、平台条款规避、资金代收、自动退款/删除/公开发布  
> **付款路径：**Upwork 所有金额以 USD 列示：Project Catalog 输入 US$575（报告统一按 US$1=¥7 折算约 ¥4,025；执行日以平台/银行实际换汇为准） 并由客户一次注资；若走自定义 fixed-price 合同，则两个里程碑为 US$288/US$287，合计 US$575，里程碑1交付基线、范围、规则和验收计划，截止 Day 17；里程碑2交付最终结果、QA、SOP和删除/回滚记录，截止 Day 28。每次只在当前里程碑 Active/Funded 后开工，提交并获批当前段后，等下一段 Active/Funded 才继续。独立获客且从未在 Upwork 建立关系的客户，可另用合规账单按 ¥4,000 报价；不得把 Upwork 客户移到站外付款。客户批准后仍有 5 天安全期。 参考扩展价：基线 ¥4,000–12,000；季度复检 ¥1,500–4,000。技术验收与注资、批准、Pending 和银行到账分开记录。  
...
- **风险：超出能力宣称安全保证**　应对：明确非渗透测试、非认证、不保证零事件，发现高风险转专业方
- **风险：改权限锁死客户**　应对：先导出管理员、准备 break-glass 账号和回滚，客户在场批准
...
- **Go：**达到本方法的商业验证门槛：“1 个客户为 10 项基线试点注资/支付 ¥4,000”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
