# 方法 05｜网站 FAQ/RAG 客服机器人

> **一页结论：**面向重复回答定价、交付、退换和兼容性问题的 SaaS、教育服务和电商，用「20–50 份文档、50 个真实问题、一个网页组件的 14 天试点」先收费验证。启动现金成本 ¥100–800，目标是 50 问命中率、引用正确率、转人工率、有效线索数。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | AI与业务自动化 |
| 买方 | 重复回答定价、交付、退换和兼容性问题的 SaaS、教育服务和电商 |
| 当前痛点 | 客服被重复问题占用，夜间访客无人承接 |
| 可交付结果 | 把批准的 FAQ 和政策做成带引用的检索问答，无法回答时收集线索并转人工 |
| 最小试点 | 20–50 份文档、50 个真实问题、一个网页组件的 14 天试点 |
| 工具栈 | Tidio/自托管知识库 + Qwen + 客户网站 |
| 启动成本 | ¥100–800（不含自己的人工） |
| 时间 | 7–14 天 MVP |
| 技能 | 知识库整理、RAG 评测、网页嵌入 |
| 参考测试报价 | 设置 ¥6,000–18,000；维护 ¥1,500–5,000/月 |
| 最小验证 | 1 个客户为 20–50 份文档和 50 问固定范围试点实际注资/支付 ¥5,000；付款只验证购买意愿，不是技术通过条件 |
| 综合分 | 4.25/5；需求证据 5 / 验证速度 3 / 低成本 4 / 复购性 5 / 自动化杠杆 5 / 获客可达 4 / 风险可控 3 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **客服被重复问题占用，夜间访客无人承接** 变成一个买方能验收的固定范围结果：**把批准的 FAQ 和政策做成带引用的检索问答，无法回答时收集线索并转人工**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。
...
Tidio 与 Intercom 已证明需求；差异是窄行业知识清理、引用、中文实施和交付后评测。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 M02、M03、M04、M07、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**网站 FAQ/RAG 客服机器人 30 天验证包
- **交付：**20–50 份文档、50 个真实问题、一个网页组件的 14 天试点；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**正式 FAQ、价格与政策、50 个真实问题、转人工邮箱、禁止回答清单
- **验收指标：**50 问命中率、引用正确率、转人工率、有效线索数
- **参考报价：**设置 ¥6,000–18,000；维护 ¥1,500–5,000/月

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 个最小试点；归一化校验：1 个该情景订单组合×¥5,000=¥5,000；模型合计=¥5,000 | ¥5,000 |
| 中性 | 中性 2 个设置单加 2 个维护；归一化校验：1 个该情景订单组合×¥26,000=¥26,000；模型合计=¥26,000 | ¥26,000 |
| 乐观 | 乐观 5 个设置单加 6 个维护；归一化校验：1 个该情景订单组合×¥72,000=¥72,000；模型合计=¥72,000 | ¥72,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 5 天；含工时口径：按首月 24 小时、目标时薪 ¥200/小时，需覆盖约 ¥5,600，按保守情景折算约 34 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「20–50 份文档、50 个真实问题、一个网页组件的 14 天试点」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**1 个客户为 20–50 份文档和 50 问固定范围试点实际注资/支付 ¥5,000；付款只验证购买意愿，不是技术通过条件
4. **技术验收门槛：**50 个真实问题中可回答问题的引用正确率至少 90%，不确定或无证据问题 100% 转人工，过期文档召回为 0；每个答案保留来源、版本和人工签字
...
|---:|---|---|---|
| Day 1 | 定边界 | Google Sheets > Blank spreadsheet；建 scope、baseline、risk 三个 tab | 写入买方“重复回答定价、交付、退换和兼容性问题的 SaaS、教育服务和电商”、固定范围“20–50 份文档、50 个真实问题、一个网页组件的 14 天试点”；产出一页范围，禁止扩到高风险动作 |
| Day 2 | 核证据 | 打开本文件“市场证据”中的全部官方链接；浏览器 > Bookmark folder | 逐条记录发布日期、事实与局限；如果关键链接失效，暂停宣传该事实 |
| Day 3 | 建 30 个 ICP | Upwork > Find Work > Search jobs > 输入 knowledge base chatbot RAG；公司官网只看 Contact/Team 通用入口 | Sheets targets 列 company/source_url/why_fit/jurisdiction/entity_type/status；只录与“客服被重复问题占用，夜间访客无人承接”直接相关的 30 个主体，不抓个人数据 |
| Day 4 | 量基线 | Google Sheets > baseline tab | 输入最近 30 天 50 问命中率、引用正确率、转人工率、有效线索数；没有数字就记录样本量、当前耗时和错误例子 |
| Day 5 | 开最小工具 | Tidio > Lyro AI Agent > Data sources；网站后台 > Theme/Code > Embed | 只开试点所需功能；栈：Tidio/自托管知识库 + Qwen + 客户网站；保存账号 owner、权限、关闭/回滚路径截图 |
| Day 6 | 收脱敏样本 | Google Drive > New > Folder > Share > Restricted | 向测试客户索取：正式 FAQ、价格与政策、50 个真实问题、转人工邮箱、禁止回答清单；密码/API key 不放文档 |
...
| Day 12 | 做第一版规则 | 浏览器无痕窗口 > 打开测试页 > 输入 50 问测试集 | 先只处理 5 条/1 页/1 个对象；字段来自“正式 FAQ、价格与政策、50 个真实问题、转人工邮箱、禁止回答清单”；保存 expected/actual/evidence，不做未经批准的生产写入 |
...
| Day 16 | 回放约定样本 | 浏览器无痕窗口 > 打开测试页 > 输入 50 问测试集 | 执行技术验收：50 个真实问题中可回答问题的引用正确率至少 90%，不确定或无证据问题 100% 转人工，过期文档召回为 0；每个答案保留来源、版本和人工签字；另记录商业验证阈值：1 个客户为 20–50 份文档和 50 问固定范围试点实际注资/支付 ¥5,000；付款只验证购买意愿，不是技术通过条件 |
...
| Day 19 | 录 90 秒证据演示 | QuickTime Player > File > New Screen Recording；或 OBS > Start Recording | 按问题20秒→20–50 份文档、50 个真实问题、一个网页组件的 14 天试点30秒→测试30秒→边界10秒；遮住账号、密钥和客户数据 |
...
| Day 22 | 确认执行范围未变 | Google Docs > Proposal > Version history；Upwork > Contract > Milestones | 对照 Day 10 已注资合同，确认“20–50 份文档、50 个真实问题、一个网页组件的 14 天试点”、技术验收、权限、日期和停止条件未变；不重新谈或重复收试点 |
| Day 23 | 运行已注资阶段 | 浏览器无痕窗口 > 打开测试页 > 输入 50 问测试集 | 自定义合同只在下一里程碑已注资后运行；Project Catalog 只在整单已注资后运行。按客户批准的最小权限先 5、再 20、再到约定上限；高风险错误、真实扣款或不可回滚变更立即停 |
| Day 24 | 每日 QA | Sheets > logs/QA tab > Create a filter | 每天抽查至少 10 条或全部小样本；只计算 50 问命中率、引用正确率、转人工率、有效线索数，保留分母、失败和不能归因部分 |
| Day 25 | 读结果 | Sheets > baseline vs pilot；Looker Studio 可选 | 只对比 50 问命中率、引用正确率、转人工率、有效线索数；写样本量和不能归因的部分 |
| Day 26 | 修一次 | Tidio/自托管知识库 + Qwen + 客户网站 > Duplicate/Clone/Version；仅在测试或副本中改 | 只修最大的一类错误；保留 v1/v2、变更说明、回放结果和恢复点，不同时改多个变量 |
...
> 网站 FAQ/RAG 客服机器人｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向重复回答定价、交付、退换和兼容性问题的 SaaS、教育服务和电商。我会在不改变生产关键动作的前提下，完成「20–50 份文档、50 个真实问题、一个网页组件的 14 天试点」，并用 50 问命中率、引用正确率、转人工率、有效线索数 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：客服被重复问题占用，夜间访客无人承接。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：把批准的 FAQ 和政策做成带引用的检索问答，无法回答时收集线索并转人工。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：设置 ¥6,000–18,000；维护 ¥1,500–5,000/月。
...
**主题：**关于贵司「网站 FAQ/RAG 客服机器人」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：客服被重复问题占用，夜间访客无人承接。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「20–50 份文档、50 个真实问题、一个网页组件的 14 天试点」，验收只看 50 问命中率、引用正确率、转人工率、有效线索数。  
...
> 补充一个具体点：本试点的最小通过条件是「1 个客户为 20–50 份文档和 50 问固定范围试点实际注资/支付 ¥5,000；付款只验证购买意愿，不是技术通过条件」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
> **项目：**网站 FAQ/RAG 客服机器人 30 天验证  
> **客户：**{客户名}  
> **范围：**20–50 份文档、50 个真实问题、一个网页组件的 14 天试点  
> **客户提供：**正式 FAQ、价格与政策、50 个真实问题、转人工邮箱、禁止回答清单  
> **交付：**基线表、实施/配置、测试证据、异常队列、SOP、回滚说明、结果复盘  
> **技术验收：**50 个真实问题中可回答问题的引用正确率至少 90%，不确定或无证据问题 100% 转人工，过期文档召回为 0；每个答案保留来源、版本和人工签字  
> **商业验证：**1 个客户为 20–50 份文档和 50 问固定范围试点实际注资/支付 ¥5,000；付款只验证购买意愿，不是技术通过条件  
> **不包含：**未授权数据、法律/医疗/金融意见、批量群发、平台条款规避、资金代收、自动退款/删除/公开发布  
> **付款路径：**Upwork 所有金额以 USD 列示：Project Catalog 输入 US$715（报告统一按 US$1=¥7 折算约 ¥5,005；执行日以平台/银行实际换汇为准） 并由客户一次注资；若走自定义 fixed-price 合同，则两个里程碑为 US$358/US$357，合计 US$715，里程碑1交付基线、范围、规则和验收计划，截止 Day 17；里程碑2交付最终结果、QA、SOP和删除/回滚记录，截止 Day 28。每次只在当前里程碑 Active/Funded 后开工，提交并获批当前段后，等下一段 Active/Funded 才继续。独立获客且从未在 Upwork 建立关系的客户，可另用合规账单按 ¥5,000 报价；不得把 Upwork 客户移到站外付款。客户批准后仍有 5 天安全期。 参考扩展价：设置 ¥6,000–18,000；维护 ¥1,500–5,000/月。技术验收与注资、批准、Pending 和银行到账分开记录。  
...
先按 `Tidio > Lyro AI Agent > Data sources；网站后台 > Theme/Code > Embed` 建最小副本/测试环境，再按 `浏览器无痕窗口 > 打开测试页 > 输入 50 问测试集` 跑 5 条正常样本、1 条空值、1 条重复、1 条权限拒绝和 1 条回滚。技术通过条件：50 个真实问题中可回答问题的引用正确率至少 90%，不确定或无证据问题 100% 转人工，过期文档召回为 0；每个答案保留来源、版本和人工签字 任何涉及发送、付款、退款、删除、公开发布、法律/医疗/金融判断的动作保持人工批准。


### 结构化提示词（先在脱敏样本上运行）

```text
你是“网站 FAQ/RAG 客服机器人”的质量辅助器，不是最终决策者。

业务目标：把批准的 FAQ 和政策做成带引用的检索问答，无法回答时收集线索并转人工
目标客户：重复回答定价、交付、退换和兼容性问题的 SaaS、教育服务和电商
允许输入：正式 FAQ、价格与政策、50 个真实问题、转人工邮箱、禁止回答清单
验收指标：50 问命中率、引用正确率、转人工率、有效线索数
...
- **风险：幻觉政策或价格**　应对：只能从批准文档检索并显示来源；找不到就转人工
- **风险：知识库过期**　应对：设置版本、负责人和每月变更检查
...
- **Go：**达到本方法的商业验证门槛：“1 个客户为 20–50 份文档和 50 问固定范围试点实际注资/支付 ¥5,000；付款只验证购买意愿，不是技术通过条件”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
