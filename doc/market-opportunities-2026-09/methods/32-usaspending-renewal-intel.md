# 方法 32｜USAspending 续约与 Incumbent 情报

> **一页结论：**面向政府 BD 顾问和 8(a)/小承包商，用「一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告」先收费验证。启动现金成本 ¥100–600，目标是 可研究账户数、来源完整率、客户标记为有效的信号比例。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 公共数据情报 |
| 买方 | 政府 BD 顾问和 8(a)/小承包商 |
| 当前痛点 | 不知道哪些机构、合同和现有供应商即将形成复购研究窗口 |
| 可交付结果 | 从公开奖项形成机构×供应商×金额×周期表，标注推断的续约窗口并回链 |
| 最小试点 | 一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告 |
| 工具栈 | USAspending API + Python/Sheets + Looker Studio |
| 启动成本 | ¥100–600（不含自己的人工） |
| 时间 | 10–14 天 |
| 技能 | API、联邦奖项结构、数据建模、谨慎推断 |
| 参考测试报价 | ¥1,500–5,000/报告；¥699–1,999/月 |
| 最小验证 | 做 20 条样报并访谈 3 个 BD；1 人预付 ¥1,500 才做自动化 |
| 综合分 | 3.95/5；需求证据 4 / 验证速度 3 / 低成本 5 / 复购性 4 / 自动化杠杆 5 / 获客可达 3 / 风险可控 4 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **不知道哪些机构、合同和现有供应商即将形成复购研究窗口** 变成一个买方能验收的固定范围结果：**从公开奖项形成机构×供应商×金额×周期表，标注推断的续约窗口并回链**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。

### 当前市场证据

- **D02｜[USAspending API](https://api.usaspending.gov/docs/)：**美国财政部 V2 Spending by Award API 支持 POST 筛选 award type、time period、agency、NAICS 和字段，并以 page_metadata.hasNext 指示分页。 **使用边界：**time_period 是奖项搜索过滤期，不是未来续约窗口；历史奖项只能形成需人工核验的研究信号，不能表述为采购承诺。
- **D24｜[USAspending Spending by Award API contract](https://github.com/fedspendingtransparency/usaspending-api/blob/master/usaspending_api/api_contracts/contracts/v2/search/spending_by_award.md)：**官方 API contract 给出 spending_by_award POST body、允许 fields、sort/order、limit/page 与 page_metadata.hasNext。 **使用边界：**generated_internal_id 可用于 USAspending award 回链；字段和 award type code 必须按当前 contract 复核，续约窗口是分析推断。
...
大型政府情报平台覆盖全面；机会是固定机构/NAICS 的轻量定制。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 D02、D24、D01、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**USAspending 续约与 Incumbent 情报 30 天验证包
- **交付：**一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**机构、NAICS、金额阈值、周期窗口、排除 award types
- **验收指标：**可研究账户数、来源完整率、客户标记为有效的信号比例
- **参考报价：**¥1,500–5,000/报告；¥699–1,999/月

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 份报告；归一化校验：1 个该情景订单组合×¥1,500=¥1,500；模型合计=¥1,500 | ¥1,500 |
| 中性 | 中性 4 份报告加 2 订阅；归一化校验：1 个该情景订单组合×¥8,500=¥8,500；模型合计=¥8,500 | ¥8,500 |
| 乐观 | 乐观 8 份报告加 8 订阅；归一化校验：1 个该情景订单组合×¥28,000=¥28,000；模型合计=¥28,000 | ¥28,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 12 天；含工时口径：按首月 18 小时、目标时薪 ¥200/小时，需覆盖约 ¥4,200，按保守情景折算约 84 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**做 20 条样报并访谈 3 个 BD；1 人预付 ¥1,500 才做自动化
4. **技术验收门槛：**在约定样本/页面上复测“可研究账户数、来源完整率、客户标记为有效的信号比例”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。
...
|---:|---|---|---|
| Day 1 | 定一条窄线 | Google Sheets > Blank > scope tab | 写唯一买方“政府 BD 顾问和 8(a)/小承包商”、唯一数据主题和试点“一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告”；不先建泛平台 |
| Day 2 | 查许可 | 打开本文件全部官方来源 > Terms/API docs | 记录允许用途、署名/声明、限流、第三方权利和更新时间；许可不清时不收费、不处理真实数据、不交付，先取得书面分类 |
| Day 3 | 跑官方精确请求 | Postman > New > HTTP Request > POST > https://api.usaspending.gov/api/v2/search/spending_by_award/ > Body > raw > JSON | 原样粘贴本文件 USAspending JSON，点 Send；保存前 20 条 generated_internal_id，并用 https://www.usaspending.gov/award/{id}/ 回查 |
...
| Day 5 | 做人工样本 | Sheets > 抽查 10 个 award_id > 打开官方 award page 对账 | 人工筛 20 条并逐条回链；把“真有用/无关/不确定”作为黄金标签 |
| Day 6 | 写匹配规则 | Sheets > rules tab | 输入：机构、NAICS、金额阈值、周期窗口、排除 award types；把必须条件、加分项、排除词和人工核验写成可见规则 |
| Day 7 | 保存分页/增量契约 | Postman > Body > raw > JSON > page；Response > page_metadata.hasNext | 按本文件请求把 page=1,2...，到 hasNext=false；保存 collection。time_period 不得写成未来续约期，推断列必须 analyst_inference=yes |
...
| Day 10 | 出首份样报 | Google Docs > New from blank | 只放 5–15 条最相关结果；页首写用途、来源和局限；结果目标：从公开奖项形成机构×供应商×金额×周期表，标注推断的续约窗口并回链 |
...
| Day 14 | 列 20 个买方 | Upwork > Find Work > Search jobs > 输入 USAspending contract research；Sheets > targets | 记录 company/source_url/why_fit/jurisdiction/entity_type；只用公司级通用入口，不抓个人邮箱或社媒数据 |
...
| Day 17 | 核验首个付费阶段已注资 | Upwork > Offers/Your active contracts > Fixed-price > 当前 milestone/Project Catalog order > status | 客户侧完成购买或注资；卖方只在当前段显示 Active/Funded 后接受/开工，绝不代客户点击 Fund。商业验证：做 20 条样报并访谈 3 个 BD；1 人预付 ¥1,500 才做自动化；未注资只保留样报，不继续做软件或批量采集 |
| Day 18 | 重做筛选 | Sheets > rules tab > Duplicate v1 to v2 | 只改最大误报来源；用原 20 条黄金集重新算精确率/召回率并保留 v1 |
| Day 19 | 启动付费实时周 | Sheets > 抽查 10 个 award_id > 打开官方 award page 对账 | 按官方更新频率手工/官方导出运行；每次记录筛选、响应/导出、条数、失败、版本和抓取日 |
...
| Day 22 | 确认下一段已注资再看使用 | Upwork > Contract > Milestones；Sheets > customer feedback tab | 自定义合同只在下一里程碑为 Active/Funded 后继续；询问哪些结果被打开/采取行动，只看 可研究账户数、来源完整率、客户标记为有效的信号比例，不把外部成交全部归因给报告 |
...
> USAspending 续约与 Incumbent 情报｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向政府 BD 顾问和 8(a)/小承包商。我会在不改变生产关键动作的前提下，完成「一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告」，并用 可研究账户数、来源完整率、客户标记为有效的信号比例 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：不知道哪些机构、合同和现有供应商即将形成复购研究窗口。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：从公开奖项形成机构×供应商×金额×周期表，标注推断的续约窗口并回链。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：¥1,500–5,000/报告；¥699–1,999/月。
...
**主题：**关于贵司「USAspending 续约与 Incumbent 情报」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：不知道哪些机构、合同和现有供应商即将形成复购研究窗口。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「一个机构/NAICS、过去三年、20 个 incumbent 信号的一次性报告」，验收只看 可研究账户数、来源完整率、客户标记为有效的信号比例。  
...
> 补充一个具体点：本试点的最小通过条件是「做 20 条样报并访谈 3 个 BD；1 人预付 ¥1,500 才做自动化」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
- **风险：奖项被误解为未来采购**　应对：所有续约窗口标为推断信号，绝不写成确定机会
- **风险：联系 incumbent 的个人**　应对：只交公司和官方奖项信息，不拼接个人邮箱或画像
...
- **Go：**达到本方法的商业验证门槛：“做 20 条样报并访谈 3 个 BD；1 人预付 ¥1,500 才做自动化”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
