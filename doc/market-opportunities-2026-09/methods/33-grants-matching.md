# 方法 33｜Grants.gov 资助匹配与截止日提醒

> **一页结论：**面向小型非营利组织、高校实验室和资助顾问，用「一个项目画像、7 天监测、10 条人工核验机会」先收费验证。启动现金成本 ¥0–300，目标是 匹配被接受率、漏报、每周节省小时、按时决策数。这里的报价和收益是**本报告的测试模型，不是行业均价或收益保证**。
...
|---|---|
| 分类 | 公共数据情报 |
| 买方 | 小型非营利组织、高校实验室和资助顾问 |
| 当前痛点 | 官方机会很多，资格和截止日筛选耗时 |
| 可交付结果 | 按组织类型、主题、地区和金额过滤，交付资格摘录、截止日、材料清单和原链接 |
| 最小试点 | 一个项目画像、7 天监测、10 条人工核验机会 |
| 工具栈 | Grants.gov Search/Saved Search + Sheets/Calendar；API 仅作已验证扩展 |
| 启动成本 | ¥0–300（不含自己的人工） |
| 时间 | 5–10 天 |
| 技能 | API、资助资格阅读、事实摘要、截止管理 |
| 参考测试报价 | ¥399–1,499/月；定制匹配报告 ¥1,000–3,500 |
| 最小验证 | 给 3 个组织各做 5 条样报；至少 1 个以 ¥799 预订一个月，否则更换细分 |
| 综合分 | 4.70/5；需求证据 5 / 验证速度 4 / 低成本 5 / 复购性 5 / 自动化杠杆 5 / 获客可达 4 / 风险可控 5 |

## 1. 为什么现在能赚钱

赚钱逻辑不是“AI/数据/模板很火”，而是把 **官方机会很多，资格和截止日筛选耗时** 变成一个买方能验收的固定范围结果：**按组织类型、主题、地区和金额过滤，交付资格摘录、截止日、材料清单和原链接**。先用人工和低成本工具交付，客户确认价值后才把重复步骤自动化；这样现金投入低，也避免先做没人买的软件。

### 当前市场证据

- **D03｜[Grants.gov API](https://www.grants.gov/api)：**Grants.gov 提供公开资助机会搜索 API，search2 可用于发现机会。 **使用边界：**匹配不等于有资格或会获资助，最终以原公告为准。
- **D17｜[Instrumentl pricing](https://www.instrumentl.com/pricing)：**Instrumentl 的 2026 年公开 Discover 计划为每月 299 美元（年付）或 349 美元（月付），包含机会匹配和截止日跟踪。 **使用边界：**这是单一竞品的公开价格和功能，不代表所有非营利组织的预算或购买意愿。
- **D18｜[Grants.gov Subscribe to Saved Searches](https://www.grants.gov/help/connect/subscribe-to-saved-searches)：**登录 Grants.gov 后可按 funding instrument、eligibility、sub-agency 等条件保存搜索并接收机会邮件通知，也可查看、修改或删除搜索。 **使用边界：**官方提醒只做条件匹配；付费服务必须增加组织级资格复核、材料缺口、垂直筛选或双语解释。
...
Grants.gov 账号内已有 saved-search 邮件提醒，Instrumentl 公开起价 299 美元/月；付费空间只在组织级资格人工复核、材料缺口、垂直筛选或双语解读，不得转售免费提醒。因此不要卖“我会某个工具”，要卖一条窄结果、真实回放、人工审批、可回滚交付和后续维护。

**证据依赖提醒：**本方法使用来源 D03、D17、D18、M30、P22、P23、P27、P28、P29、P31、P32、P35。它们支持市场/渠道/工具事实，但不直接证明你的细分客户会购买；付费意愿必须由本方案的预售试点验证。

## 2. 产品、价格与单位经济

### 固定范围产品

- **名称：**Grants.gov 资助匹配与截止日提醒 30 天验证包
- **交付：**一个项目画像、7 天监测、10 条人工核验机会；另附基线、测试记录、异常清单、SOP、回滚/删除说明。
- **客户输入：**组织类型、主题、地区、金额、资格、截止窗口、排除词
- **验收指标：**匹配被接受率、漏报、每周节省小时、按时决策数
- **参考报价：**¥399–1,499/月；定制匹配报告 ¥1,000–3,500

### 月收益情景（税前可计收入；数字平台按文中分成/版税模型）

| 情景 | 本报告假设 | 预估月营收 |
|---|---|---:|
| 保守 | 保守 1 个预售；归一化校验：1 个该情景订单组合×¥799=¥799；模型合计=¥799 | ¥799 |
| 中性 | 中性 10 个 ¥799 订阅；归一化校验：1 个该情景订单组合×¥7,990=¥7,990；模型合计=¥7,990 | ¥7,990 |
| 乐观 | 乐观 20 个订阅加 4 份定制报告；归一化校验：1 个该情景订单组合×¥28,000=¥28,000；模型合计=¥28,000 | ¥28,000 |

- **回本周期：**现金口径：按保守月营收匀速折算约 12 天；含工时口径：按首月 18 小时、目标时薪 ¥200/小时，需覆盖约 ¥3,900，按保守情景折算约 147 天。这是容量模型；真实回本以实际收款日、平台结算期、退款、税和工时为准。
- **毛利闸门：**试点结束统计实际工时、工具费、平台费、退款与支持。税前贡献毛利低于 60% 时，不扩量，先提价或缩范围。
- **停止条件：**30 天无付费、关键验收失败、平台/KYC 不可用、数据许可不清或必须靠违规抓取/群发才能获客，立即停止或换细分。

## 3. 最小验证方案

1. 不先做完整产品；只做「一个项目画像、7 天监测、10 条人工核验机会」。
2. 使用公开信息或客户主动提供的脱敏样本，不先索要管理员、支付或生产写权限。
3. **商业验证门槛：**给 3 个组织各做 5 条样报；至少 1 个以 ¥799 预订一个月，否则更换细分
4. **技术验收门槛：**在约定样本/页面上复测“匹配被接受率、漏报、每周节省小时、按时决策数”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。
...
|---:|---|---|---|
| Day 1 | 定一条窄线 | Google Sheets > Blank > scope tab | 写唯一买方“小型非营利组织、高校实验室和资助顾问”、唯一数据主题和试点“一个项目画像、7 天监测、10 条人工核验机会”；不先建泛平台 |
| Day 2 | 查许可 | 打开本文件全部官方来源 > Terms/API docs | 记录允许用途、署名/声明、限流、第三方权利和更新时间；许可不清时不收费、不处理真实数据、不交付，先取得书面分类 |
| Day 3 | 取 10–20 条官方样本 | Grants.gov > Login > Connect > Subscribe to Saved Searches > Add Saved Search > 填 eligibility/agency/关键词 > Save | 优先用官方 UI、Saved Search、CSV 或 Postman；保存筛选条件、日期、状态码/导出名和 official ID，不假设通用脚本支持该来源 |
...
| Day 5 | 做人工样本 | Grants.gov > Search Grants > 用 opportunity number 回查资格、截止日和原公告；Sheets 记录材料缺口 | 人工筛 20 条并逐条回链；把“真有用/无关/不确定”作为黄金标签 |
| Day 6 | 写匹配规则 | Sheets > rules tab | 输入：组织类型、主题、地区、金额、资格、截止窗口、排除词；把必须条件、加分项、排除词和人工核验写成可见规则 |
| Day 7 | 保存官方筛选/请求 | Grants.gov > Login > Connect > Subscribe to Saved Searches > Add Saved Search > 填 eligibility/agency/关键词 > Save | 保存官方 Saved Search、导出参数或 Postman collection；POST body、分页、认证和限流按该来源官方文档逐项记录，不用一个通用命令冒充全覆盖 |
...
| Day 10 | 出首份样报 | Google Docs > New from blank | 只放 5–15 条最相关结果；页首写用途、来源和局限；结果目标：按组织类型、主题、地区和金额过滤，交付资格摘录、截止日、材料清单和原链接 |
...
| Day 13 | 发布固定价服务 | Upwork > Find Work > Your services > Create Project | Upwork Price 字段输入 US$115（报告统一按 US$1=¥7 折算约 ¥805；执行日以平台/银行实际换汇为准）；粘贴标题、样报、来源许可和免责声明，先卖人工报告，不卖未完成订阅软件 |
| Day 14 | 列 20 个买方 | Upwork > Find Work > Search jobs > 输入 grant eligibility research；Sheets > targets | 记录 company/source_url/why_fit/jurisdiction/entity_type；只用公司级通用入口，不抓个人邮箱或社媒数据 |
| Day 15 | 首批 10 个触达 | Upwork > 打开匹配职位 > Apply now；或通过司法辖区闸门后 Gmail > Compose | 逐封附 3 条公开样报；真实身份、实体地址、隐私告知和退订齐全，不用追踪像素 |
| Day 16 | 访谈并进入合法合同路径 | Calendly > Event types > 20 min；普通职位 > Apply now 后等待客户 Offer；仅已付款旧客户或主动从 Project Catalog 发消息的客户 > Propose new contract | 访谈 3 人：上次错过什么、哪些字段最值钱、谁批准；Upwork 总价输入 US$115（报告统一按 US$1=¥7 折算约 ¥805；执行日以平台/银行实际换汇为准）。普通职位客户必须由客户发送 Offer，卖方不能把 Propose new contract 当通用按钮；技术验收与付款分开 |
| Day 17 | 核验首个付费阶段已注资 | Upwork > Offers/Your active contracts > Fixed-price > 当前 milestone/Project Catalog order > status | 客户侧完成购买或注资；卖方只在当前段显示 Active/Funded 后接受/开工，绝不代客户点击 Fund。商业验证：给 3 个组织各做 5 条样报；至少 1 个以 ¥799 预订一个月，否则更换细分；未注资只保留样报，不继续做软件或批量采集 |
| Day 18 | 重做筛选 | Sheets > rules tab > Duplicate v1 to v2 | 只改最大误报来源；用原 20 条黄金集重新算精确率/召回率并保留 v1 |
| Day 19 | 启动付费实时周 | Grants.gov > Search Grants > 用 opportunity number 回查资格、截止日和原公告；Sheets 记录材料缺口 | 按官方更新频率手工/官方导出运行；每次记录筛选、响应/导出、条数、失败、版本和抓取日 |
...
| Day 22 | 确认下一段已注资再看使用 | Upwork > Contract > Milestones；Sheets > customer feedback tab | 自定义合同只在下一里程碑为 Active/Funded 后继续；询问哪些结果被打开/采取行动，只看 匹配被接受率、漏报、每周节省小时、按时决策数，不把外部成交全部归因给报告 |
...
> Grants.gov 资助匹配与截止日提醒｜先做固定范围试点，用真实数据验收，不承诺虚假增长

**副标题（直接粘贴）**

> 面向小型非营利组织、高校实验室和资助顾问。我会在不改变生产关键动作的前提下，完成「一个项目画像、7 天监测、10 条人工核验机会」，并用 匹配被接受率、漏报、每周节省小时、按时决策数 做前后验收。涉及发送、付款、退款、删除、公开发布或高风险判断的步骤默认保留人工批准。

**服务说明（直接粘贴）**

> 你现在可能遇到的问题是：官方机会很多，资格和截止日筛选耗时。本项目不会先卖一套昂贵系统，而是先交付一个可回滚试点：按组织类型、主题、地区和金额过滤，交付资格摘录、截止日、材料清单和原链接。你会收到现状基线、配置/数据文件、测试记录、异常清单、操作 SOP、回滚办法和 14/30 天结果复盘。固定范围外的工作会在开始前单独报价。参考价：¥399–1,499/月；定制匹配报告 ¥1,000–3,500。
...
**主题：**关于贵司「Grants.gov 资助匹配与截止日提醒」的一页试点建议

> 你好，{姓名/团队}：  
> 我查看了贵司公开的 {页面/流程/职位信息}，发现一个可以用固定范围验证的问题：官方机会很多，资格和截止日筛选耗时。我不是来承诺排名或收入的；我可以先用公开信息或你提供的脱敏样本，做「一个项目画像、7 天监测、10 条人工核验机会」，验收只看 匹配被接受率、漏报、每周节省小时、按时决策数。  
...
> 补充一个具体点：本试点的最小通过条件是「给 3 个组织各做 5 条样报；至少 1 个以 ¥799 预订一个月，否则更换细分」。如果你已有团队在做，我也可以只交只读审计和测试清单；若不相关，回复“不需要”，我会停止联系。
...
> **项目：**Grants.gov 资助匹配与截止日提醒 30 天验证  
> **客户：**{客户名}  
> **范围：**一个项目画像、7 天监测、10 条人工核验机会  
> **客户提供：**组织类型、主题、地区、金额、资格、截止窗口、排除词  
> **交付：**基线表、实施/配置、测试证据、异常队列、SOP、回滚说明、结果复盘  
> **技术验收：**在约定样本/页面上复测“匹配被接受率、漏报、每周节省小时、按时决策数”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。  
> **商业验证：**给 3 个组织各做 5 条样报；至少 1 个以 ¥799 预订一个月，否则更换细分  
> **不包含：**未授权数据、法律/医疗/金融意见、批量群发、平台条款规避、资金代收、自动退款/删除/公开发布  
> **付款路径：**Upwork 所有金额以 USD 列示：Project Catalog 输入 US$115（报告统一按 US$1=¥7 折算约 ¥805；执行日以平台/银行实际换汇为准） 并由客户一次注资；若走自定义 fixed-price 合同，则两个里程碑为 US$58/US$57，合计 US$115，里程碑1交付基线、范围、规则和验收计划，截止 Day 17；里程碑2交付最终结果、QA、SOP和删除/回滚记录，截止 Day 28。每次只在当前里程碑 Active/Funded 后开工，提交并获批当前段后，等下一段 Active/Funded 才继续。独立获客且从未在 Upwork 建立关系的客户，可另用合规账单按 ¥799 报价；不得把 Upwork 客户移到站外付款。客户批准后仍有 5 天安全期。 参考扩展价：¥399–1,499/月；定制匹配报告 ¥1,000–3,500。技术验收与注资、批准、Pending 和银行到账分开记录。  
...
先按 `Grants.gov > Login > Connect > Subscribe to Saved Searches > Add Saved Search > 填 eligibility/agency/关键词 > Save` 建最小副本/测试环境，再按 `Grants.gov > Search Grants > 用 opportunity number 回查资格、截止日和原公告；Sheets 记录材料缺口` 跑 5 条正常样本、1 条空值、1 条重复、1 条权限拒绝和 1 条回滚。技术通过条件：在约定样本/页面上复测“匹配被接受率、漏报、每周节省小时、按时决策数”，每项都有输入、预期、实际、证据链接与人工签字；付款只验证购买意愿，不作为技术通过条件。 任何涉及发送、付款、退款、删除、公开发布、法律/医疗/金融判断的动作保持人工批准。
...
- **风险：错误资格或承诺获批**　应对：引用公告原文，明确非申请保证且最终由组织核验
- **风险：只搬运免费数据**　应对：必须加入项目画像、资格摘录、材料清单和 no-fit 原因
...
- **Go：**达到本方法的商业验证门槛：“给 3 个组织各做 5 条样报；至少 1 个以 ¥799 预订一个月，否则更换细分”；关键验收达标；贡献毛利可接受；交付不依赖违规或单点人工英雄主义。
