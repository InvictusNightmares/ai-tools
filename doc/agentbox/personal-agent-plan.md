...
将当前 Windows 云电脑改造成无桌面 Linux Agent 主机，用于代码修改、测试、构建、调研和文档等任务。日常从实体电脑通过 `ssh root@agentbox` 接管，不再依赖 Windows 桌面。
...
不复制 `.codex`、`auth.json`、会话数据库或缓存。临时 root 密码、SSH 私钥、Tailscale auth key 和节点凭据不得进入 Git、聊天或日志。
...
- 本机为单人使用，唯一管理账号为 `root`，不创建 `agent` 用户；SSH 始终只接受公钥。
- OpenSSH TCP 22 只允许从 `tailscale0` 进入，只接受 `root` 的 Ed25519 公钥。
...
- 所有 Agent、数据库、任务队列、浏览器自动化和项目服务都放在 `/srv/agentbox/<stack>` 的 Compose 栈中。管理员以 `root` 执行 `docker compose ...`；容器自身的运行 UID 与宿主机管理账号分开。
...
8. 首次启动后执行 `bootstrap-debian.sh`；生产 7898 通过 HTTPS 检查后才切换日常 APT/shell，并启用自动更新，然后建立 `root` 公钥 + Tailscale + SSH，安装并加固 Docker，创建容器专用代理网络。
9. 实体电脑验证两条独立 root 公钥 SSH 会话，然后执行 `finalize-debian.sh` 锁定 root 密码，保留 root 公钥登录。
...
- 实体电脑可使用 `ssh root@agentbox`，公网接口不能访问 TCP 22。
- root 仅允许公钥 SSH；SSH 密码/交互认证和 Tailscale SSH 关闭；手机不持有 SSH 私钥。
...
- 强制执行一次 `update-agentbox-proxy --force` 能成功刷新；故意提供无效候选时不会替换最后可用的生产配置。
